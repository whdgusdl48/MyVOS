...
class QueryTransformerBlock(nn.Module):
    def __init__(self, model_cfg: DictConfig):
        ...
        self.inject_object= CrossAttention(self.embed_dim,
                                            self.num_heads,
                                            add_pe_to_qkv=[False, False, False])
        self.ffn2 = FFN(self.embed_dim, self.ff_dim)


        self.inject_from_mask = CrossAttention(self.embed_dim,
                                            self.num_heads,
    ...
    def forward(
        ...
        object_features = object_features.flatten(3, 4).flatten(0, 1).transpose(1, 2).contiguous()
        ### First
        ### learnable parameter q, mask_value kv attention

        object_f, o_weight = self.inject_object(object_features,
                                                pixel_flat,
                                                mask_pe,
                                                pixel_pe
                                                )
        object_f = self.ffn2(object_f)
        msk_value, p_weights = self.inject_from_mask(msk_value,
                                             pixel_flat,
                                             mask_pe,
                                             pixel_pe,
                                             attn_mask=None,
        ...
        pixel_object, p_weights = self.out_from_object(pixel_flat,
                                             object_f,
                                             pixel_pe,
                                             mask_pe,
                                             attn_mask=None,
                                             need_weights=need_weights)
        pixel_object = self.pixel_ffn2(pixel, pixel_object)
        ...
        pixel_semantic, p_weights = self.out_from_mask(pixel_flat,
                                             ...
                                             attn_mask=None,
                                             need_weights=need_weights)
        pixel_semantic = self.pixel_ffn1(pixel, pixel_semantic)
        pixel_flat = pixel_object + pixel_semantic
        msk_value = msk_value.view(b, n, c, h, w)
        object_f = object_f.contiguous().view(b, n, c, 1, 1)
        return pixel_flat, object_f, msk_value, p_weights, p_weights
...
class QueryTransformer(nn.Module):
    ...
    def forward(self,
        ...
        for i in range(self.num_blocks):
            pixel, object_features, msk_value, q_weights, p_weights = self.blocks[i](    pixel,
                                                                        pixel_pe,
                                                                        msk_value,
                                                                        mask_pe,
                                                                        object_features,
