...
class QueryTransformerBlock(nn.Module):
    ...
    def forward(
            ...
            cur_obj_init : torch.Tensor,
            cur_obj_emb : torch.Tensor,
        ...
        x, q_weights = self.read_from_pixel(cur_obj_init,
                                            msk_value,
                                            cur_obj_emb,
                                            mask_pe,
                                            attn_mask=None,
                                            need_weights=need_weights)
        x = self.self_attn(x, cur_obj_emb)
        x = self.ffn(x)
        ...
        pixel_flat = self.pixel_ffn(pixel, pixel_object + pixel_semantic)

        return pixel_flat, x, p_weights, p_weights
...
class QueryTransformer(nn.Module):
    ...
    def forward(self,
        ...
        cur_obj_init = self.query_init.weight.unsqueeze(0).expand(bs * num_objects, -1, -1)
        cur_obj_emb = self.query_emb.weight.unsqueeze(0).expand(bs * num_objects, -1, -1)
    
        for i in range(self.num_blocks):
            pixel, cur_obj_init, q_weights, p_weights = self.blocks[i](    pixel,
                                                                           pixel_pe,
                                                                           msk_value,
                                                                           mask_pe,
                                                                           cur_obj_init,
